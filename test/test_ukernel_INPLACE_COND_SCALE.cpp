...
void test_correctness_FLOAT_INPLACE_COND_SCALE_TILE(void)
{
...
void test_correctness_FLOAT_INPLACE_COND_SCALE_END(void)
{
#if defined(SMALL_HAS_FLOAT_SUPPORT)
    using BufferT = FloatBuffer;
    ...
    FLOAT_LOAD_END_C(output_buf.data(), FLOAT_W_ob - 1, FLOAT_C_ob);

    FLOAT_INPLACE_COND_SCALE_END_C(negative_buf, c_cur, FLOAT_W_ob - 1, FLOAT_C_ob);

    FLOAT_STORE_END_C(output_buf.data(), FLOAT_W_ob - 1, FLOAT_C_ob);
    //==================================================

    for (dim_t ii = 0; ii < FLOAT_W_ob - 1; ++ii)
    {
        for (dim_t jj = 0; jj < FLOAT_C_ob; ++jj)
        {
            size_t ix = ii*FLOAT_C_ob + jj;
...
TEST_LIST = {
    {"correctness FLOAT_INPLACE_COND_SCALE_TILE",
     small::float_detail::test_correctness_FLOAT_INPLACE_COND_SCALE_TILE},
     {"correctness FLOAT_INPLACE_COND_SCALE_END",
     small::float_detail::test_correctness_FLOAT_INPLACE_COND_SCALE_END},
    {NULL, NULL}
};
