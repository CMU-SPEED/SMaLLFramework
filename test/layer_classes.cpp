//------------------------------------------------------------------------------
// mobilenet_components.cpp - Scotts attempt at OO design for SMaLL
//------------------------------------------------------------------------------
...
//****************************************************************************
template <typename BufT=float>
class Layer
{
public:
    Layer() {};

    virtual void compute_output(std::vector<BufT> const &input,
                                std::vector<BufT> &output) = 0;
private:
};
...
public:
    ...
    add_layer(Layer<BufT> *new_layer)  // call with std::move??
    {
        m_layers.push_back(new_layer);
    }

    std::vector<BufT> compute_outputs(std::vector<BufT> const &input)
    {
        if (m_layers.empty()) throw std::exception("No layers in model.");

        std::vector<BufT> out(m_layers[0]->comput_output(input);
        for (layer: m_layers)
        {
...
//****************************************************************************
template <typename WeightT=float, typename BufT=float>
class DepthwiseConv2D : public Layer<BufT>
{
public:
    Layer(uint32_t input_height, uint32 input_width,
          uint32_t num_input_channels, uint32_t num_output_channels,
          uint32_t kernel_width, uint32_t kernel_height,
          uint32_t stride,
          uint8_t  t_pad, uint8_t b_pad, uint8_t l_pad, uint8_t r_pad,
          std::vector<WeightT> const &filter_params)
        : m_num_input_channels(num_input_channels),
          m_num_output_channels(num_output_channels),
          m_kernel_width(kernel_width),
          m_kernel_height(kernel_height),
          m_stride(stride),
          m_t_pad(t_pad), m_b_pad(b_pad), m_l_pad(l_pad), m_r_pad(r_pad),
          m_filter_params(filter_params),
          m_output(output_dim(input_height + t_pad + b_pad, stride, kernel_height) *
                   output_dim(input_width  + l_pad + r_pad, stride, kernel_width))
    {
    }

    virtual compute_output(std::vector<BufT> const &input,
                           std::vector<BufT> &output)
    {
    }

private:
    uint32_t const m_num_input_channels, m_num_output_channels;
    uint32_t const m_kernel_width, m_kernel_height;
    uint32_t const m_stride;
    uint8_t  const m_t_pad, m_b_pad, m_l_pad, m_r_pad;
    std::vector<WeightT> const m_filter_params;

    std::vector<BufT> m_output;
};
