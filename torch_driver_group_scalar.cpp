...
int main(int argc, char **argv)
{
  ...
  a = torch::mul(a, 1.0);
  torch::Tensor test_weights = torch::randn(C_o_1 * C_i * kernel_size * kernel_size).reshape({C_i, C_o_1, kernel_size, kernel_size});
...

  {
    ...
    for (int run = 0; run < RUNS; run++)
    {
      ...
      t0 = rdtsc();
      // std::cout << W_ob << "  " << C_ob << "  " << C_ib << "  " << stride << "  " << channel_stride << "  " << kernel_size << "  " << kernel_size << " K " << C_o_1 << " C " << C_i << " G " << C_i / C_o_1 <<std::endl;
      direct_convolution<W_ob, C_ob, C_o_1, stride, channel_stride, kernel_size, kernel_size>( C_o_1, C_o_1, C_i / C_o_1, N, M, input_dc, filter_dc, out_intermediate_dc);
      t1 = rdtsc();
      MIN(sum_pool, (t1 - t0));
