...
@tf.function(experimental_compile=True)
def custom_pool(inter_tensor):
    pool_out = pool(inter_tensor)
    return pool_out
...
warm_out_block = custom_block(input_tensor)
# print(warm_out_block.shape)
...
for i in range(1000):
    st = count()
    out_block = custom_conv(input_tensor)
    out = custom_pool(out_block)
    et = count()
