    ...
    float *O)
{
  ...
  uint32_t W_o_pool_full = 1 , H_o_pool = 1;
...
#pragma omp parallel for
  ...
  for (uint32_t j = 0; j < C; j += C_ob)
  {
    ...
    // }
    gpool_kernel_end(I_ptr, H_o, W_o_full, O_ptr);
...
// THIS IS THE SAME AS CHANNEL BLOCK FUSED (since the stencil reduces the entire height)
...
#pragma omp parallel for
  ...
  for (uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    for (uint32_t l = 0; l < H_o - 1 ; l++)
    {
      ...
      // for (uint32_t k = 0; k < W_o_pool; k += W_ob_pool)
      // {
      //   row_gpool_kernel<pool_stride * C_ob, pool_stride, pool_H_f, pool_W_f>(O_conv_ptr, l, k, pool_col_stride, O_pool_ptr, H_o);
      //   O_conv_ptr += pool_stride * W_ob * C_ob;
      // }
      row_gpool_kernel_end(O_conv_ptr, W_o_full, O_pool_ptr);
    }
    uint32_t l = H_o - 1;
    uint32_t col_offset = l * W_o_full * C_ob;
    uint32_t input_col_offset = (l * stride) * W_i * C_ob + input_block_offset;
    ...
    //   O_conv_ptr += pool_stride * W_ob * C_ob;
    // }
    row_gpool_kernel_last(O_conv_ptr, H_o, W_o_full, O_pool_ptr);
  }
}
...
#pragma omp parallel for
  ...
  for (uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    for (uint32_t l = 0; l < H_o - 1; l++)
    {
      ...
      for (uint32_t k = 0; k < W_o; k += W_ob)
      {
        fused_conv_pool_kernel<stride * C_ob, H_f, W_f>(W_i * C_ib, I_ptr, filter_block_ptr, O_ptr, O_pool_ptr);
        I_ptr += stride * W_ob * C_ob;
        O_ptr += W_ob * C_ob;
      }
      fused_conv_pool_kernel_end<stride * C_ob, H_f, W_f>(W_i * C_ib, I_ptr, filter_block_ptr, O_ptr, O_pool_ptr);
    }
    ...
    fused_conv_pool_kernel_last<stride * C_ob, H_f, W_f>(W_i * C_ib, I_ptr, filter_block_ptr, O_ptr, H_o, W_o_full, O_pool_ptr);
  }
}
