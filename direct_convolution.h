...
void fused_pooling_direct_convolution(
  ...
  uint32_t W_o_pool = 0;
  op_dim(W_o, pooling_stride, pooling_kernel, W_o_pool);
  // buffer is reused for different blocks
  ...
  for(uint32_t j = 0; j < C_o; j += C_ob){
    ...
    for(uint32_t l = 0; l < H_o; l++){
      ...
      // max the entire row (should be in L1)
      for(uint32_t p_k = 0 ; p_k < W_o_pool; p_k+= POOL_UNROLL){
        int i = 0;
      }
    }
  }
