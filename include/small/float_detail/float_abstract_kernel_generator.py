...
#operation set up
vector_instruction = vector_instruction_table[sys.argv[2]] # One of the Keys in the vector instruction table
...
reg2_unlabeled = "{op}".format(op= operand_order[1]) + "_{idxs[1][0]}_{idxs[1][1]}" if num_operands >=2 else None
reg3_unlabeled = "{op}".format(op= operand_order[2]) + "_{idxs[2][0]}_{idxs[2][1]}" if num_operands ==3 else None



vector_instruction_instance = vector_instruction.format(reg1=reg1_unlabeled, reg2 = reg2_unlabeled, reg3=reg3_unlabeled)
...
for kk in range(W_ob):
    ...
    for jj in range(C_ob//vector_instruction_table["SIMD"]):
        ...
        for operand in range(num_operands):
            register_operand_idxs.append([kk % order_of_operands[operand_order[operand]][0], jj % order_of_operands[operand_order[operand]][1]])

        s += ['c_{k}_{j} = {vop};\\'.format(vop=vector_instruction_instance.format(idxs = register_operand_idxs), k=kk, j=jj)]
        # else:
