...
public:
    ...
    DoubleBuffer() :
        ...
        m_buffer(nullptr)
    {
        //std::cerr << "DoubleBuffer::default_ctor " << (void*)this << std::endl;
    }

    DoubleBuffer(size_t num_elts) : m_num_elts(num_elts)
    {
        ...
        //std::cerr << "DoubleBuffer::ctor " << (void*)this
    ...
    {
        //std::cerr << "DoubleBuffer copy ctor\n";
        if (0 != posix_memalign((void**)&m_buffer,
                                64,
    ...
    DoubleBuffer(DoubleBuffer&& other) noexcept
        : m_num_elts(0),
          m_buffer(nullptr)
    {
        //std::cerr << "DoubleBuffer move ctor\n";
    ...
    {
        //std::cerr << "DoubleBuffer copy assignment\n";
        if (this != &other)
        {
            // expensive, but with exception guarantees.
            DoubleBuffer tmp(other);
    ...
    {
        //std::cerr << "DoubleBuffer move assignment\n";
        if (this != &other)
        {
            m_num_elts = 0;
            free(m_buffer);
    ...
    ~DoubleBuffer()
    {
        // std::cerr << "DoubleBuffer::dtor " << (void*)this
    ...
    inline void swap(DoubleBuffer &other)
    {
        if (this != &other)
        {
            std::swap(m_buffer, other.m_buffer);
