    ...
    BufferT */*__restrict__*/ O)
{
    ...
    if constexpr (op_type == OP_CONV || op_type == OP_LEAKY_RELU || op_type == OP_EWISE_MUL_SCALAR || op_type == OP_EWISE_ADD_SCALAR) // if (F != nullptr)
    {
        F_buf = F->data();
    }
...
#pragma omp parallel num_threads(N)
#endif
    {
        ...
        for (index_t g = group_start; g < group_end; g++)
        {
            ...
            if constexpr ((op_type == OP_LEAKY_RELU) || (op_type == OP_EWISE_MUL_SCALAR) || (op_type == OP_EWISE_ADD_SCALAR))
            {
                F_group = F_buf;
            }
            else
...
#endif
            {
                ...
                {
                    bool first = (rewrite_output || op_type == OP_EWISE_ADD_SCALAR) && (i == 0); // TODO: Generalize this condition
