...
#include <small/double_detail/compute_with_padding.hpp>
    ...
    ScalarT const * F_a = NULL)
{
    ...
    if (first)
    {
        DOUBLE_ZERO_END_C(l_pad_el, _C_ob);
    }
    else
    {
        DOUBLE_LOAD_END_C(O_ptr, l_pad_el, _C_ob);
    }

    if constexpr(fused_single_element_before == OP_UPSAMPLE)
    {
        DOUBLE_ACCUM_END_C_upsample(F_b, _stride_before, _C_ib, l_pad_el, _C_ob);
    }
    ...
    {
        double norm = 1.0 / (1.0 * F_h * F_w);
        DOUBLE_DIV_END_C(c_tile, norm, l_pad_el, _C_ob);
    }
    ...
    DOUBLE_STORE_END_C(O_ptr, l_pad_el, _C_ob);
    O_ptr += _G_b * _K_b;
}
