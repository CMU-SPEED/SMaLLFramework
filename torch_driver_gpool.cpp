...
#include "src/fused_global_pooling.h"
// conv fused dw conv
...
int main(int argc, char ** argv)
{
  ...
  float * dw_w_ptr = test_weights.data_ptr<float>();
  ...
  auto conv_dw = torch::nn::AdaptiveAvgPool2d(torch::nn::AdaptiveAvgPool2dOptions({1, 1}));
  ...
  uint64_t pool_ops = out.numel()              * (atol(argv[3])*atol(argv[4])*.0);
  uint64_t effective_conv_h = (out.size(2) - 1) * 2 + 3;
  uint64_t effective_conv_w = (out.size(3) - 1) * 2 + 3;

  uint64_t effective_conv_ops = conv_ops;
  ...
  uint64_t t0, t1, sum=ULLONG_MAX, sum_pool=ULLONG_MAX, sum_fused = ULLONG_MAX;
  //unfused
  {
    ...
    for (int run = 0; run < RUNS; run++){
      ...
      MIN(sum,(t1 - t0));
      t0 = rdtsc();
      gpooling(C_o, out_intermediate_dimensions[2], out_intermediate_dimensions[3] ,out_intermediate_dc, out_dc);
      t1 = rdtsc();
      MIN(sum_pool,(t1 - t0));
    ...
    for (int run = 0; run < RUNS; run++){
      ...
      t0 = rdtsc();
      l_fused_gpool_conv_direct_convolution<stride,kernel_size, kernel_size >(C_i, C_o, N, M, input_dc, filter_dc, out_intermediate_dc, out_dc);
      // direct_convolution_pooling_aware<stride,kernel_size, kernel_size >(C_i, C_o, N, M, input_dc, filter_dc, out_intermediate_dc);
      t1 = rdtsc();
      MIN(sum_fused,(t1 - t0));
    ...
    printf("%d %d %d ",out_intermediate_dimensions[2], out_intermediate_dimensions[3] , C_o );
    printf("%.2f %.2f\n",(sum_pytorch*1.0)/sum_fused, ((sum+sum_pool)*1.0)/sum_fused);
