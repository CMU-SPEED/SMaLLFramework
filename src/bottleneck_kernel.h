...
inline void bottleneck_kernel(
                            uint32_t input_col_stride,
                            float * I,
                            float * F,
                            float * O){
    float *b = F;
    LOAD_TILE_C_strided(I, step, W_ob_bottleneck, C_ob);
    MUL_TILE_C(b, W_ob_bottleneck, C_ob);
    int updates = 0;
    ...
    for(uint32_t m = 1; m < W_f; m++){
        ...
        bottleneck_TILE_C(step, a, b, W_ob_bottleneck, C_ob);
        b += C_ob;
    ...
    STORE_TILE_C(O, W_ob_bottleneck, C_ob);
}

template <uint32_t step, uint32_t H_f, uint32_t W_f>
inline void bottleneck_kernel_end(
                            uint32_t input_col_stride,
                            float * I,
                            float * F,
                            float * O,
    ...
    float * b = F; 
    LOAD_LAST_C_strided(I, step, W_ob_bottleneck, C_ob, W_last);
  ...
  // int count = 0;
    for(uint32_t m = 1; m < W_f; m++){
        ...
        bottleneck_END_C(step, a, b, W_last, C_ob);
        b += C_ob;
...
//             b+=C_ob;
//         }
//         STORE_TILE_C_POOL(O_pool_ptr, W_ob_bottleneck, C_ob);
//     }
...
//                                         (O_pool_col) * C_ob;
//             float * b = F_bottleneck + n_p*(pool_W_f)*C_ob;
//             LOAD_TILE_C_POOL(O_pool_ptr, W_ob_bottleneck, C_ob);
...
// template <uint32_t step, uint32_t pool_stride, uint32_t pool_H_f, uint32_t pool_W_f>
// void row_bottleneck_kernel_end(
//     float *I,
//     uint32_t O_row,
//     uint32_t O_pool_col,
//     uint32_t pool_col_stride,
...
//             b += C_ob;
//         }
//         STORE_END_C_POOL(O_pool_ptr, W_ob_bottleneck, C_ob, W_last);
//     }
//     for (uint32_t n_p = 1; n_p < pool_H_f; n_p++)
//     {
//         if ((O_row - n_p) % pool_stride == 0 && (int)(O_row - n_p) >= 0 && (O_row + pool_H_f - (n_p + 1)) < H_o)
...
//                             (O_pool_col)*C_ob;
//             float *b = F_bottleneck + n_p * (pool_W_f)*C_ob;
//             LOAD_LAST_C_POOL(O_pool_ptr, W_ob_bottleneck, C_ob, W_last);
//             for (uint32_t m = 0; m < pool_W_f; m++)
//             {
//                 int input_stencil_w = m * C_ob;
//                 float *a = I + input_stencil_w;
...
// void fused_conv_bottleneck_kernel(
//     uint32_t input_col_stride,
//     float *I,
//     float *F,
//     float *O,
...
//     //Fused pooling
//     bottleneck_TILE_IP(pool_col_stride, W_ob, C_ob, pool_stride, pool_H_f, pool_W_f, F_bottleneck, O_row, O_col, O_pool, H_o, W_o_full);
// }

// template <uint32_t step, uint32_t H_f, uint32_t W_f, uint32_t pool_stride, uint32_t pool_H_f, uint32_t pool_W_f>
// inline void fused_conv_bottleneck_kernel_end(
//     uint32_t input_col_stride,
//     float *I,
//     float *F,
//     float *O,
...
//     bottleneck_END_IP(pool_col_stride, W_last, C_ob, pool_stride, pool_H_f, pool_W_f, F_bottleneck, O_row, O_col, O_pool, H_o, W_o_full);
// }
