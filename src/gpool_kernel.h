...
template<uint32_t C>
inline void init_O(float * O)
{
    for(uint32_t kk = 0; kk < C; kk++)
    {
        O[kk] = 0;
    }
}
inline void gpool_kernel_end(
    float *I,
    uint32_t H_f, 
    uint32_t W_f_full,
    float *O)
{
    uint32_t W_f = (W_f_full / W_ob_g) * W_ob_g;
    uint32_t W_last = W_f_full % W_ob_g;
    ZERO_TILE_C(W_ob_g, C_ob);
    // printf("init: %.4f %.4f %.4f  \n", c_tile[0], c_tile[16], c_tile[32]);

    float * I_row_ptr = I;
    for(uint32_t n = 0; n < H_f; n++) 
    {
        float * I_ptr = I_row_ptr;
        for(uint32_t m = 0 ; m < W_f; m += W_ob_g)
        {
            ADD_TILE_C_G(I_ptr, W_ob_g, C_ob);
            // printf("add: %.4f %.4f %.4f  \n", I_ptr[0], I_ptr[16], I_ptr[32]);
            // printf(" %.4f %.4f %.4f  \n", c_tile[0], c_tile[16], c_tile[32]);

            I_ptr += W_ob_g*C_ob;
        }
        ADD_LAST_C_G(I_ptr, W_last, C_ob);

        I_row_ptr += (W_f_full) * C_ob;
    }

    float div_factor = 1.0/(H_f * W_f_full);
    // printf("reduced : %.4f %.4f %.4f %.4f \n", c_tile[0], c_tile[16], c_tile[32], div_factor);
    REDUCE_div_C(O, div_factor, W_ob_g, C_ob);
}

// //Sub-Stencil fused pooling kernels
inline void row_gpool_kernel_end(float *I,
                                uint32_t W_f_full,
                                float *O)
{
    uint32_t W_f = (W_f_full / W_ob_g) * W_ob_g;
    ...
    ADD_LAST_C_G(I_ptr, W_last, C_ob);
    REDUCE_C(O, W_ob_g, C_ob);
}

inline void row_gpool_kernel_last(float *I,
                                uint32_t H_f,
                                uint32_t W_f_full,
                                float *O)
{
