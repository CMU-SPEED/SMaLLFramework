...
template <uint32_t stride, uint32_t H_f, uint32_t W_f>
 void l_fused_pooling_direct_convolution(
  uint32_t C_i,
  uint32_t C_o,
  // uint32_t H_f,
  // uint32_t W_f,
  ...
  for(uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    //Last 3x3 Input Channel Block
    {
      ...
      //First 3x3 Output Row
      {
        ...
        uint32_t pool_col_offset = pool_block_offset;
      ...
      for(uint32_t l = POOL_STRIDE; l < H_o-(POOL_KERNEL-1); l+= POOL_STRIDE)
      {
        //Even 3x3 Output Row
        {
          ...
          uint32_t pool_col_offset = pool_row_offset;
...
template <uint32_t stride, uint32_t H_f, uint32_t W_f>
 void j_fused_pooling_direct_convolution(
  uint32_t C_i,
  uint32_t C_o,
  // uint32_t H_f,
  // uint32_t W_f,
...
template <uint32_t stride, uint32_t H_f, uint32_t W_f>
 void j_fused_H_tile_pooling_direct_convolution(
  uint32_t C_i,
  uint32_t C_o,
  // uint32_t H_f,
  // uint32_t W_f,
  ...
  for(uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    float *O_pool = O + pool_block_offset;
    //First 3x3 Input Channel Block
    {
      // These are all 0
    uint32_t input_block_offset = (0/C_ib)*H_i*W_i*C_ib;
    ...
    //Last 3x3 Input Channel Block
    {
      // printf(" channel block %d: %d %d\n", i, offset, block_offset);
        ...
        pool_3_rows<POOL_STRIDE*C_ob>(W_o*C_ob, W_o, O_buffer, O_pool);
        uint32_t pool_row_offset = W_o_pool*C_ob + pool_block_offset;
        uint32_t pool_row = 1;
        float * O_conv_row = O_buffer + (pool_row*POOL_STRIDE)*W_o*C_ob;
        float * O_pool_row = O_pool + pool_row*W_o_pool*C_ob;
        //Third to Penultimate 3x3 Output Rows
        for(uint32_t l = POOL_KERNEL; l < H_o ; l+= POOL_STRIDE)
        {
          pool_3_rows<POOL_STRIDE*C_ob>(W_o*C_ob, W_o, O_conv_row, O_pool_row);
          pool_row_offset += W_o_pool*C_ib;
          pool_row++;
          O_pool_row += W_o_pool*C_ib;
          O_conv_row += (POOL_STRIDE)*W_o*C_ob;
 ...
 void H_tile_fused_pooling_direct_convolution(
  ...
  for(uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    //Last 3x3 Input Channel Block
    {
      ...
      uint32_t pool_row_offset = pool_block_offset + W_o_pool*C_ob;
      uint32_t pool_row = 1;
      float * O_pool_row = O + pool_row_offset;
      float * O_conv_row = O_buffer + (pool_row * POOL_STRIDE)*W_o*C_ob;
      ...
      for(uint32_t l = POOL_KERNEL; l < H_o; l+= POOL_STRIDE)
      {
         ...
         //conv 2 rows
        for(uint32_t ll = 0; ll < POOL_STRIDE; ll++)
        {
        ...
        //Pool 3 rows starting with the conv output from previous iteration
        pool_3_rows<POOL_STRIDE*C_ob>(W_o*C_ob, W_o, O_conv_row, O_pool_row);


        pool_row_offset += W_o_pool*C_ob;
        O_conv_row += (POOL_STRIDE)*W_o*C_ob;
...
template <uint32_t stride, uint32_t H_f, uint32_t W_f>
 void H_loop_fused_pooling_direct_convolution(
  uint32_t C_i,
  uint32_t C_o,
  // uint32_t H_f,
  // uint32_t W_f,
  ...
  for(uint32_t j = 0; j < C_o; j += C_ob)
  {
    ...
    //Start H_tile
    {
      ...
      float * filter_block_ptr = F + filter_i_c_block;
        //3x3 Output Rows
        for(uint32_t l = 0; l < POOL_KERNEL; l++)
        {
    ...
    float * O_pool_row = O_pool + W_o_pool*C_ob;
    uint32_t pool_row = 1;
    for(uint32_t l = POOL_KERNEL; l < H_o; l+=POOL_STRIDE)
    {
      uint32_t offsets[2];
