  ...
  for (uint32_t j = 0; j < C_o; j += _C_ob)
  {
    ...
    for (uint32_t l = 0; l < H_o; l++)
    {
      ...
      for (uint32_t k = 0; k < W_o; k += _W_ob)
      {
        ...
        dw_kernel<_W_ob, _C_ob, _C_ib, stride * _C_ob, H_f, W_f>(W_i * _C_ib, I_ptr, filter_block_ptr, O_ptr);

        I_ptr += stride * _W_ob * _C_ob;
        O_ptr += _W_ob * _C_ob;
      }
      // printf("%.2f %.2f %.2f %.2f \n", I_ptr[0], I_ptr[16], I_ptr[32], filter_block_ptr[0]);
      dw_kernel_end<_W_ob, _C_ob, _C_ib, stride * _C_ob, H_f, W_f>(W_i * _C_ib, I_ptr, filter_block_ptr, O_ptr, W_last);
    }
...
//     float *F_ptr = F_dw + (j/C_ob) * pool_H_f * pool_W_f * C_ob;
...
//       {
//         dw_kernel<pool_stride * C_ob, pool_H_f, pool_W_f>(input_row_pool_stride, I_ptr, F_ptr, O_ptr);
//         I_ptr += pool_stride * W_ob_dw * C_ob;
//         O_ptr += W_ob_dw * C_ob;
//       }
//       dw_kernel_end<pool_stride * C_ob, pool_H_f, pool_W_f>(input_row_pool_stride, I_ptr, F_ptr, O_ptr, W_pool_last);
...
// 			for (uint32_t k = 0; k < W_o_pool; k += W_ob_dw)
// 			{
//         row_dw_kernel<pool_stride * C_ob, pool_stride, pool_H_f, pool_W_f>(O_conv_ptr, l, k, pool_col_stride, F_ptr, O_pool_ptr, H_o);
//         O_conv_ptr += pool_stride*W_ob*C_ob;
// 			}
//       row_dw_kernel_end<pool_stride * C_ob, pool_stride, pool_H_f, pool_W_f>(O_conv_ptr, l, W_o_pool, pool_col_stride, F_ptr, O_pool_ptr, H_o, W_pool_last);
//     }
// 	}
// }
...
//       for (uint32_t k = 0; k < W_o; k += W_ob)
//       {
//         fused_conv_dw_kernel<stride * C_ob, H_f, W_f, pool_stride, pool_H_f, pool_W_f>(W_i * C_ib, I_ptr, filter_block_ptr, O_ptr, l, k, pool_col_stride, F_ptr, O_pool_ptr, H_o, W_o_full);
//         I_ptr += stride * W_ob * C_ob;
//         O_ptr += W_ob * C_ob;
//       }
//       fused_conv_dw_kernel_end<stride * C_ob, H_f, W_f, pool_stride, pool_H_f, pool_W_f>(W_i * C_ib, I_ptr, filter_block_ptr, O_ptr, W_last, l, W_o, pool_col_stride, F_ptr, O_pool_ptr, H_o, W_o_full);
//     }
//   }
// }
