...
//   op_dim(W_o, DW_STRIDE, DW_KERNEL, W_o_pool);
//   op_dim(H_o, DW_STRIDE, DW_KERNEL, H_o_pool);
//
...
//       // Last 3x3 Output Row
//       {// 3x3 params
//         uint32_t col_offset = (pool_row*DW_STRIDE + DW_STRIDE)*W_o*C_ob;
//         uint32_t input_col_offset = ((pool_row*DW_STRIDE + DW_STRIDE) * stride)*W_i*C_ob + input_block_offset;
//
//         float * I_ptr = I + input_col_offset;
//         //first tile
//         {
...
inline void dw_convolution_kernel_first_start(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_first(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_first_end(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_start(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_end(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_last_start(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_last(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
inline void dw_convolution_kernel_last_end(
  uint32_t input_col_stride,
  uint32_t pool_col_stride,
  float * I,
  float * F,
...
{
  const int w_block = 3;
  constexpr uint32_t s = DW_STRIDE;
  uint32_t W_o , H_o;
  op_dim(W_i, DW_STRIDE, DW_KERNEL, W_o);
  op_dim(H_i, DW_STRIDE, DW_KERNEL, H_o);
  // printf("\n %d %d to %d %d\n",H_i, W_i,H_o, W_o);
  uint32_t offset = 0;
  #pragma omp parallel for
  for(uint32_t i = 0; i < C; i+=C_ob)
  {
    ...
    for(uint32_t l = 0; l < H_o; l++)
    {
      ...
      for(uint32_t k = 0; k < W_o; k+=w_block)
      {
        ...
        c0 = _mm256_load_ps(I_ptr + (0 * s)*C_ob);
        c1 = _mm256_load_ps(I_ptr + (0 * s)*C_ob + SIMD);
        c2 = _mm256_load_ps(I_ptr + (1 * s)*C_ob);
        c3 = _mm256_load_ps(I_ptr + (1 * s)*C_ob + SIMD);
        c4 = _mm256_load_ps(I_ptr + (2 * s)*C_ob);
        c5 = _mm256_load_ps(I_ptr + (2 * s)*C_ob + SIMD);
        c6 = _mm256_load_ps(I_ptr + (3 * s)*C_ob);
        c7 = _mm256_load_ps(I_ptr + (3 * s)*C_ob + SIMD);
        c8 = _mm256_load_ps(I_ptr + (4 * s)*C_ob);
        c9 = _mm256_load_ps(I_ptr + (4 * s)*C_ob + SIMD);
        c10 = _mm256_load_ps(I_ptr + (5 * s)*C_ob);
        c11 = _mm256_load_ps(I_ptr + (5 * s)*C_ob + SIMD);

        for(uint32_t n = 0; n < DW_KERNEL; n++){

          int filter_offset_h = n*W_f*C_ob;
          int input_stencil_h = /*input_col_offset +*/ n * input_col_stride /*+ input_row_offset*/;
