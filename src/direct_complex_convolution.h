...
template <uint32_t _W_ob, uint32_t _C_ob, uint32_t _C_ib, uint32_t stride, uint32_t channel_stride, uint32_t H_f, uint32_t W_f>
void direct_convolution(
  ...
  #endif
  for(uint32_t j = 0; j < C_o*COMPLEX; j += _C_ob){
    ...
    fflush(0);
    for(uint32_t i = 0; i < C_f*COMPLEX; i += _C_ib){
      ...
      for(uint32_t l = 0; l < H_o; l++)
      {
          ...
          for(uint32_t k = 0; k < W_o; k += _W_ob){
            ...
            if(_C_ib == 1){ 
              ...
              complex_conv_kernel<_W_ob, _C_ob, _C_ib, stride * _C_ob, H_f, W_f>(W_i * _C_ib, I_ptr, filter_block_ptr, O_ptr);
            }
        ...
        if(_C_ib == 1){
          ...
          complex_conv_kernel_end<_W_ob, _C_ob, _C_ib, stride * _C_ob, H_f, W_f>(W_i * _C_ib, I_ptr, filter_block_ptr, O_ptr, W_last);
        }
      }
