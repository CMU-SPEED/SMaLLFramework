...
int main(int argc, char **argv)
{
    ...
    test_weights_dw = torch::mul(test_weights_dw, 1.0 / (1.0 * pool_kernel_size * pool_kernel_size*C_o_1));
    float *w_ptr = a.data_ptr<float>();
    float *c_ptr = test_weights_dw.data_ptr<float>();
    ...
    // {
    //     for(uint32_t k = 0; k < C_o_1; k++)
    //     {
    //         for (uint32_t h = 0; h < pool_kernel_size; h++)
    //         {
    //             for (uint32_t w = 0; w < pool_kernel_size; w++)
    //             {
    //                 *c_ptr *= (c) *(pool_kernel_size * pool_kernel_size) + (w+1);
    //                 c_ptr++;
    //             }
    //         }
    //     }
    ...
    auto pool = torch::nn::Conv2d(torch::nn::Conv2dOptions(C_o, C_o_1*(C_o/C_o_1), pool_kernel_size).stride(pool_stride).padding(0).groups(C_o/C_o_1).bias(false));
    ...
    torch::Tensor weights_dw = pool->weight.reshape({C_o, C_o_1, pool_kernel_size, pool_kernel_size});
...

    {
        ...
        // std::cout<<filter_dw_dimensions<<std::endl;
        copy_torch2dc<C_ob, C_ib>(weights_dw, 'd', filter_dw_dimensions, filter_dw_dc);
        for (int run = 0; run < RUNS; run++)
        {
            ...
            // MIN(sum,(t1 - t0));
            // t0 = rdtsc();
            direct_group_convolution<W_ob, C_ob, C_o_1, pool_stride, pool_kernel_size, pool_kernel_size>(C_o_1, C_o, output_rows, output_cols, out_intermediate_dc, filter_dw_dc, out_dc);
            t1 = rdtsc();
            MIN(sum_pool, (t1 - t0));
        ...
        bool check_dw = check_eqivalence<C_ob, C_ib>(out, 'o', out_dimensions, out_dc, LIMIT);
        assert(check_dw == 1);
        // print_cycles(sum);
